...
QtObject {
    ...
    property Timer _daemonQueryTimeout: Timer {
        interval: 3000; repeat: false
        onTriggered: {
            if (!_savedState) return
            console.warn("Daemon query timed out — restoring layout with fresh shells")
            _performRestore()
        }
    }
    ...
    function saveSessionState() {
        ...
        // Skip saving if no window has meaningful content (multiple tabs,
        // splits, or was restored from a previous session).
        var worthSaving = false
        for (var i = 0; i < windows.length; i++) {
            if (windows[i].isWorthSaving()) { worthSaving = true; break }
        }
        if (!worthSaving) {
            appSettings.storage.setSetting("_SESSION_STATE", "")
            return
