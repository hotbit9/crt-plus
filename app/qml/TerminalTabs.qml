...
Item {
    ...
    function isWorthSaving() {
        if (tabsModel.count > 1) return true
        var rootPane = tabRepeater.itemAt(0)
        if (!rootPane) return false
        if (rootPane.hasMultipleLeaves()) return true
        if (rootPane.hasAnyActivity()) return true
        return false
    }
    ...
    Timer {
        ...
        onTriggered: {
            ...
            terminalWindow._wasRestored = true
